<template>
	<view class="content">
		<view class="connectProcess" v-if="connectProcess != 'ok'">{{connectProcess}}</view>
				...
				try {
					...
					if (Object.keys(store.state.msgs).length == 0) {
						...
						for (let toUid in store.state.msgs) {
							...
							let text = lastMsg.data.text.length > 24 ?
								lastMsg.data.text.slice(0, 24) + '...' : lastMsg.data.text
